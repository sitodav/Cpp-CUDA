...
void matMatGPUv3(int *a,int *b,int *c,int l,int m,int n){
	...
	//siccome voglio (differentemente da quanto fatto sulla slide) allocare dinamicamente con dimensioni scelte a runtime, i 2 array in shared memory:
	//innanzitutto saranno array monodimensionali, e per allocarli dinamicamente, siccome si puo' indicare solo un size di allocazione ...(vedi appunti su dropbox)
	__shared__ extern int buffer[];
	int *buffa=&buffer[0]; //buffa utilizzerà blockDim.x * blockDim.x elementi (anche se può andare oltre in effetti entrando nell'area assegnata a buffb)
	int *buffb=&buffer[blockDim.x * blockDim.x]; //e lo stesso buffb
	
	int globx=blockIdx.x * blockDim.x + threadIdx.x;
	int globy=blockIdx.y * blockDim.y + threadIdx.y;
	int astart=blockIdx.x * blockDim.x * m;
	int bstart=blockIdx.y * blockDim.x;
	int cumsum=0;
	
	for(int as = astart, bs= bstart; as<=astart+m-1; as+= blockDim.y, bs+=blockDim.y*n){
		
		buffa[threadIdx.x * blockDim.y + threadIdx.y]= a[as+ threadIdx.x * m + threadIdx.y];
		buffb[threadIdx.x * blockDim.x + threadIdx.y] = b[bs+threadIdx.x * n +threadIdx.y];
		__syncthreads();
		for(int i=0;i<blockDim.x;i++)
			cumsum+=buffa[threadIdx.x*blockDim.x + i]*buffb[i*blockDim.x +threadIdx.y];
		__syncthreads();
	}
...
matMatGPUv3<<<sizeGriglia,sizeBlocco,2* sizeBlocco.x * sizeBlocco.x >>>(da,db,dc,l,m,n);
